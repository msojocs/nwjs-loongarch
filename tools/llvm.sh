...
if [ ! -d "$project_dir" ]; then
  ...
  cd "$source_dir"
  git clone -b llvmorg-19.1.7 https://github.com/llvm/llvm-project.git
fi
...
# git checkout origin/llvmorg-19.1.7
mkdir -p $build_dir
...
if [ ! -f "$llvm_dir/lib/cmake/llvm/LLVMConfigExtensions.cmake" ];then
  cd $project_dir
  cmake -S llvm -B build -G "Ninja" -DCMAKE_BUILD_TYPE=Release -DLLVM_ENABLE_PROJECTS="clang;lld" -DCMAKE_INSTALL_PREFIX=$llvm_dir
  cd "$build_dir"
