...
//----replace
     ...
     */
    #undef LSS_REG
    #define LSS_REG(ar,a) register int64_t __r##ar __asm__("$a"#ar) = (int64_t)a// loongarch64
    /* syscall is like subroutine calls, all caller-saved registers may be
     * clobbered, we should add them to the |Clobbers| list.
     * a0 is not included because it's in the output list.
     */
    #define LSS_SYSCALL_CLOBBERS "$t0", "$t1", "$t2", "$t3", "$t4", "$t5", "$t6",    \// loongarch64
                                 "$t7", "$t8", "memory"// loongarch64
    #undef  LSS_BODY
    #define LSS_BODY(type,name,args...)                                       \
